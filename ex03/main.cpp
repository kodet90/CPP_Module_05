...
int	main() {
	...
	std::cout << "[Intern: shrubbery creation]" << std::endl;
	{
		try
		{
			...
			Intern	someRandomIntern;
			Form *	rrf = someRandomIntern.makeForm("shrubbery creation", "Bender");
	...
	std::cout << std::endl;
	std::cout << "[Intern: wrong name]" << std::endl;
	{
		try
		{
			Intern	someRandomIntern;
			Form *	rrf = someRandomIntern.makeForm("wrong name", "Bender");
			delete rrf;
		}
		catch (std::exception & e) {
			std::cout << e.what() << std::endl;
